...
│   │   ├── dataRefresh.js     ← Data refresh: two schedulers (QUICK 15-min clock-aligned + 95-min lookback; DAILY 17:30 ET + 24-h lookback), both all 16 CME symbols; refreshSymbol(sym, lookbackMinutes) + refreshAll(lookbackMinutes); emits refresh_start/progress/complete events for WS broadcast; HP recompute on each full refresh; >24h lookbacks delegate to backfillHistoricalWindow.js (v14.40, renamed from dailyRefresh.js v14.26)
...
| v14.39 | Right-panel + OHLC-options-row stale-on-symbol-switch fix (Track 3 follow-up) — extends the v14.5 clear-on-click discipline to the Market Context / Directional Bias / Setup Score / Macro Context / Conviction subcomponents + the OHLC header options-metrics row. [public/js/alerts.js](public/js/alerts.js) `_clearSymbolState(newSym)` now bumps a `window._symbolSwitchGen` counter, invalidates every window-scoped cache the bias renderers read from (`_lastMacroScore`/`_lastReadinessStatus`/`_lastBlockedGateIds`/`_lastBiasDirection`/`_lastSetupData`/`_lastSetupScore`/`_lastGammaData`), and blanks the bias panel + options/gamma/DD-band widgets synchronously. New sym-btn click handler fires the clear + bias/options refetch on the same tick as the click (no longer gated on the chart candle round-trip). `_fetchOptionsData` short-circuits for symbols with no ETF proxy (MHG) and all bias/options/DD-band fetches gained a per-await generation guard so stale responses from a prior symbol cannot overwrite the new blank state. Client-only fix; B9 paper-trading edge unaffected (no server, scoring, outcome-resolution, or backtest changes). `public/sw.js` cache bumped `v37 → v38`. See CHANGELOG [v14.39]. | ✅ Complete |
| v14.39.1 | Hotfix on top of v14.39 — bias panel stuck on `Loading…` after symbol switch. Root cause: listener-order race. The bias IIFE's `chartViewChange` / `dashModeChange` listeners ([alerts.js:3727/3731](public/js/alerts.js#L3727)) register synchronously at script load and fire FIRST; the alerts IIFE's listeners ([alerts.js:1231](public/js/alerts.js#L1231)) register inside async `boot()` and fire SECOND, so their `_clearSymbolState` bumps `_symbolSwitchGen` AFTER the bias fetch already captured the pre-bump value — bias fetch then marks itself stale and bails. Options / DD Band widgets worked because the alerts IIFE also calls `_fetchOptionsData` / `_fetchDDBands` post-bump; bias was the only widget missing that post-bump kick. **Fix:** add `window._fetchAndRenderBias(activeSymbol)` call to both the `chartViewChange` and `dashModeChange` handlers in the alerts IIFE, after the bump, mirroring the v14.39 sym-btn click handler's pattern. `public/sw.js` cache bumped `v38 → v39`. Hypothesis about `window._fetchAndRenderBias` being undefined was wrong — [alerts.js:3310](public/js/alerts.js#L3310) does set it. Client-only; B9 paper-trading edge unaffected. See CHANGELOG [v14.39.1]. | ✅ Complete |
| v14.41 | Data trust audit + remediation pass — overnight unattended audit of the data foundation before calculation-adjustment work. 10-category read-only audit (`scripts/dataAudit.js`) → targeted remediation → re-audit → cross-validation. Phase 1 status **red** (1A 1C 1F). Fixes: [server/trading/simulator.js:258](server/trading/simulator.js#L258) `_persistForwardTrade` dxyDirection fallback reversed to `dxy.direction → breadth.dollarRegime → null` (aligned with [setups.js:1319-1321](server/analysis/setups.js#L1319-L1321); motivated by M6E seed-gap: live breadth returned `dollarRegime='flat'` for ~5h post-restart, shadowing the real DXY signal); 7-day all-16-symbol Databento re-backfill cleared 35 dupe files (94 dupes max) on 2026-04-21..24; MGC 2026-04-06 negative-OHLC spread bars (209) dropped; 39 null-breadth forward trades on 2026-04-20/21 backfilled from breadth cache with `_backfilledFields` provenance; breadth cache extended +4 dates; VIX extended +19 dates. Phase 3 **yellow** (1A 1H green; 1F green). Phase 4: backtest deterministic (4C bit-identical ×3); backfill audit 10/10 verified (4D); live-vs-backtest parity 1/10 off-by-1 (4B); **4A regression shift — 13 trades vs v14.40 claimed 21 — same config/engine/data; cannot reproduce v14.40 reference, new profile**. Standing items for Jeff: HP coverage 2026-04-01+ gap; DXY 2026-04-03+ gap; historical breadth-cache 17/30 spot-check divergence; M6E seed file. B9 paper-trading edge unaffected — no engine/gating/scoring changes, only stamping layer. See [data/analysis/2026-04-24_data_trust_audit_REPORT.md](data/analysis/2026-04-24_data_trust_audit_REPORT.md). | ✅ Complete |
| v14.42 | OPRA live computation diagnosis + April HP backfill + daily HP snapshot writer — closes v14.41 standing item #1 (HP coverage 2026-04-01+ gap). Stage 1 read-only diagnosis (`scripts/diagOpraMetrics.js`) confirmed live OPRA computation path is sound; OPRA OpenInterest broadcasts once/day from OCC overnight clearing file (~06:00–06:30 ET), so post-restart `_strikeData` is empty until next morning broadcast → CBOE fallback during that window is correct. Stage 2 historical backfill: 24 OPRA zips extracted via `scripts/extractOpraZips.js`, ETF closes backfilled via DBEQ.BASIC, Phase 1e + 1f produced 96 new HP files (16 trading dates × 6 ETFs, 2026-04-02 → 2026-04-24). April backtest unblocked: **34 trades / WR 35.3% / PF 3.063 / Net +$2,894** (was 0 trades pre-backfill). Stage 3 EOS snapshot writer: `server/data/opraSnapshot.js snapshotDailyHP()` wired into `dataRefresh.refreshAll()` at 17:30 ET, writes live OPRA chain to disk in same shape as Phase 1f with `dataSource: 'opra-live'` provenance; skip-when-no-OI; manual trigger `POST /api/opra/snapshot`. DIA still has zero OPRA zips (MYM HP stays blocked). See CHANGELOG [v14.42]. | ✅ Complete |
| v14.40 | Refresh scheduler restructure + Bug 8 reference-symbol coverage — final phase of the v14.33→v14.40 data-layer remediation arc. Part A: `server/data/dailyRefresh.js` → `dataRefresh.js`; single hourly schedule split into QUICK (every 15 min, clock-aligned to `:00/:15/:30/:45`, 95-min lookback) + DAILY (17:30 ET via `Intl.DateTimeFormat('en-US', { timeZone: 'America/New_York' })` poll, 24-h lookback), both covering all 16 CME symbols. `refreshSymbol(symbol, lookbackMinutes)` default 95 preserves `dataQuality.js` back-compat; > 24-h lookbacks delegate to new `backfillSymbolWindow()` exported from `scripts/backfillHistoricalWindow.js`. Part B: `backfillHistoricalWindow.js` default pool extended to `TRADEABLE_SYMBOLS.concat(REFERENCE_SYMBOLS)` — 16 symbols; `--tradeable-only` flag for old 8-symbol behavior. Part B (run): catch-up backfill of the 8 reference symbols (M6E/M6B/MBT/ZT/ZF/ZN/ZB/UB, `--days 14`) — 95 files written, 93 166 bars merged, 570 agg files. Part C: `POST /api/refresh/*` routes accept `{ lookbackMinutes, force }` body with validation (1…43 200 min); 409 in-flight guard. Part D: `refresh_start` / `refresh_progress` / `refresh_complete` WS events (fire from manual + auto). Part E: dashboard dropdown restructured — window `<select>` + force checkbox + three live status lines; new 32-px full-width topbar banner with running / success / warning / validation-error states. `public/sw.js` cache bumped `v39 → v40`. B9 paper-trading edge unaffected — no changes to setup detection, confidence scoring, `applyMarketContext()`, outcome resolution, or the backtest engine (regression spot-check reproduced 21-trade reference profile). Bug 8 closed. See CHANGELOG [v14.40]. | ✅ Complete |
...
| `POST /api/refresh/symbol/:symbol` | Manual single-symbol refresh (v14.40). Body `{ lookbackMinutes?: 1..43200, force?: boolean }`. Defaults: 95 min, false. 409 if another refresh is in flight. |
| `POST /api/refresh/all` | Manual full refresh across all 16 CME symbols. Same body shape + validation. Emits `refresh_start`/`refresh_progress`/`refresh_complete` WS events. |
| `GET /api/refresh/status` | `{ inFlight: {scope, symbol?, lookbackMinutes, startedAt, currentIndex, totalSymbols} \| null, lastRun: {...results} \| null, lastRefreshPerSymbol: {...} }` (v14.40) |
...
- `node scripts/backfillHistoricalWindow.js [--dry-run] [--symbol SYM] [--days N] [--verbose] [--force] [--tradeable-only]` — per-symbol per-day Databento REST pull (`hist.databento.com/v0/timeseries.get_range`, `ohlcv-1m`, `GLBX.MDP3`, `stype_in=parent`) for all 16 CME symbols by default (v14.40; `--tradeable-only` restores the old 8-tradeable pool). Skip-if-complete: (symbol, date) with ≥1,300 bars is left alone; `--force` bypasses. Today always skipped (live feed is appending). Merges with Phase 2 dedup semantics, writes `time` field with `.bak` sidecars, re-aggregates 5m/15m/30m to both `futures/{sym}/{tf}/` and `futures_agg/{sym}/{tf}/`. Chunked per-day stays under Databento rate limits. Also exports `backfillSymbolWindow(symbol, lookbackMinutes, {force})` consumed by `dataRefresh.js` for manual `POST /api/refresh/*` calls with `lookbackMinutes > 1440`. Re-run anytime for gap-filling.
