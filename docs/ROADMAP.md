...
**Current version:** v14.41 — data trust audit + remediation pass (overnight audit: bar integrity, breadth cache, forward-test stamping, simulator dxyDirection fallback fix, 7-day re-backfill, VIX extension, forward-trade backfill) (2026-04-24)
...
- Data trust audit v14.41 (2026-04-24, overnight unattended): 10-category read-only audit → targeted remediation → re-audit → cross-validation → findings doc ([data/analysis/2026-04-24_data_trust_audit_REPORT.md](data/analysis/2026-04-24_data_trust_audit_REPORT.md)). Phase 1 status red (1A 1C 1F). Fixes: simulator `_persistForwardTrade` dxyDirection fallback reversed to `dxy.direction → breadth.dollarRegime → null` (aligned with setups.js:1319-1321; motivated by M6E seed-gap shadowing real DXY); 7-day all-16-symbol Databento re-backfill cleared 35 dupe files (94 dupes max) on 2026-04-21..24; MGC 2026-04-06 negative-OHLC spread bars (209) dropped; 39 null-breadth forward trades on 2026-04-20/21 backfilled from breadth cache with `_backfilledFields` provenance; breadth cache extended +4 dates; VIX extended +19 dates. Phase 3 status yellow (1A 1H green; 1F green; 1C red standing — 17/30 historical divergence flagged for Jeff). Phase 4 cross-val: backtest deterministic (4C bit-identical ×3); backfill audit 10/10 verified (4D); live-vs-backtest parity 1/10 off-by-1 (4B); **4A regression shift — 13 trades vs v14.40 claimed 21, same config, same engine, same data; cannot reproduce the v14.40 reference. New reference profile**. Standing items for Jeff: ~~HP coverage 2026-04-01+ gap~~ **closed in v14.42** (April backfill + daily snapshot writer); DXY coverage 2026-04-03+ gap; historical breadth-cache recompute decision; M6E seed file to close the live cold-start gap.
- HP coverage v14.42 (2026-04-26): closes the v14.41 HP gap end-to-end. Stage 1 read-only diagnosis confirmed live OPRA computation path is sound — OPRA OpenInterest broadcast cadence (once/day from OCC overnight clearing file, ~06:00–06:30 ET) means post-restart `_strikeData` is empty until next morning broadcast; `options.js` falls through to CBOE correctly during that window. Stage 2 historical backfill: 24 OPRA zips (statistics + definitions) extracted via `scripts/extractOpraZips.js`, ETF closes backfilled via DBEQ.BASIC, Phase 1e + 1f produced 96 new HP files (16 trading dates × 6 ETFs, 2026-04-02 → 2026-04-24). April backtest unblocked: **34 trades / WR 35.3% / PF 3.063 / Net +$2,894** (was 0 trades). Stage 3 daily HP snapshot writer: `server/data/opraSnapshot.js snapshotDailyHP()` wired into `refreshAll()` at 17:30 ET, writes live OPRA chain to disk with `dataSource: 'opra-live'` provenance — going-forward gap won't re-open. Manual trigger: `POST /api/opra/snapshot`. DIA still has zero OPRA zips (MYM HP stays blocked, pre-v14.42 state).
